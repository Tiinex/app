...
import { packageFileBytes } from '@tiinex/core/export/package.bytes.js';
import { prepareNodeHandoffManufacturingInput, enumerateNodeWorkspace } from '@tiinex/core/tooling/portable/adapters/node/handoff.manufacture.js';
...
import { inspectPortableToolingBootstrap } from '@tiinex/core/tooling/portable/handoff/toolingBootstrap.js';
import { qualifiedHandoffFixture } from '@tiinex/core/tooling/portable/handoff/qualifiedHandoffFixture.js';
import { qualifySelectedHandoffArtifact } from '@tiinex/core/tooling/portable/handoff/routeArtifactConformance.js';
...
try {
  ...
  await mkdir(path.join(workspaceRoot, '.tiinex', 'checkpoints', 'focused-tooling', 'run-1'), { recursive: true });
  await writeFile(path.join(workspaceRoot, '.tiinex', 'checkpoints', 'focused-tooling', 'run-1', 'plan.json'), '{"status":"runtime-only"}\n');
  await writeFile(path.join(workspaceRoot, '.tiinex', 'checkpoints', 'focused-tooling', 'run-1', 'receipt.json'), '{"status":"runtime-only"}\n');
  await writeFile(path.join(workspaceRoot, '.tiinex', 'checkpoint-cache.bin'), new Uint8Array([9, 8, 7]));
  ...
  assert(firstEnumeration.evidence.exclusions.directories.includes('.tiinex'), 'canonical Workspace enumeration must exclude runtime-only .tiinex state by default');
  assert.equal(firstEnumeration.materialization.entries.some((entry) => entry.path.startsWith('.tiinex/')), false, 'runtime-only .tiinex files must not enter canonical Workspace materialization');
  ...
  assert.equal(workspaceArchiveEntries.entries.some((entry) => entry.path.startsWith('.tiinex/')), false, 'manufactured canonical Workspace archive must exclude runtime checkpoint/receipt state');
